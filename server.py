"""compliance-mcp — regulatory & compliance intelligence for autonomous agents.
...
try:
    ...
    mcp.add_middleware(ReliabilityMiddleware(server_id="compliance"))
...
async def _okf_reliability_route(request):
    from starlette.responses import JSONResponse
    import okf_endpoint
    return JSONResponse(okf_endpoint.reliability_payload("compliance"))
...
async def rest_search(request: Request) -> JSONResponse:
    ...
    filters = {k: b.get(k) for k in ("industry", "agency", "regulation_type", "keyword",
                                     "severity", "days_back", "limit")}
...
async def rest_alerts(request: Request) -> JSONResponse:
    b = await _body(request)
    return _resp(await core.do_alerts(b.get("industry", ""), b.get("severity"),
...
async def rest_recall(request: Request) -> JSONResponse:
    b = await _body(request)
    return _resp(await core.do_recall(b.get("product"), b.get("company"), b.get("category"),
                                      b.get("days_back"), agent_key=_akey(request, b),
                                      payment_tx=b.get("payment_tx"), api_key=identity.bearer(request)))


@mcp.custom_route("/v1/enforcement", methods=["POST"])
async def rest_enforcement(request: Request) -> JSONResponse:
    b = await _body(request)
    return _resp(await core.do_enforcement(b.get("agency"), b.get("company"), b.get("industry"),
                                           b.get("min_penalty"), agent_key=_akey(request, b),
                                           payment_tx=b.get("payment_tx"), api_key=identity.bearer(request)))


@mcp.custom_route("/v1/deadlines", methods=["POST"])
async def rest_deadlines(request: Request) -> JSONResponse:
    b = await _body(request)
    return _resp(await core.do_deadlines(b.get("industry"), b.get("days_ahead", 30),
                                         agent_key=_akey(request, b), payment_tx=b.get("payment_tx"),
                                         api_key=identity.bearer(request)))
...
async def rest_digest(request: Request) -> JSONResponse:
    b = await _body(request)
    return _resp(await core.do_digest(b.get("industry"), b.get("jurisdiction"),
                                      agent_key=_akey(request, b), payment_tx=b.get("payment_tx"),
                                      api_key=identity.bearer(request)))
...
async def admin_aggregate(request: Request) -> JSONResponse:
    ...
    days = int(qp["days"]) if qp.get("days", "").isdigit() else None
    if qp.get("wait") == "1":
        return JSONResponse(await agg.run_aggregation(days))
    asyncio.create_task(agg.run_aggregation(days))
...
async def server_card(request: Request) -> JSONResponse:
    ...
    return JSONResponse({
        ...
        "categories": ["compliance", "regulatory", "legal", "data", "risk"],
...
# ── Background: re-aggregate every 12h ───────────────────────────────────────
async def _agg_loop():
    while True:
...
if __name__ == "__main__":
    import uvicorn
    logger.info(f"compliance-mcp starting on 0.0.0.0:{config.PORT} "
